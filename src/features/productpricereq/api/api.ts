...
const updateRequest = async (id: string, payload: ProductPriceReqPayload): Promise<ProductPriceReq> => {
    await delay(600);
    const index = requests.findIndex(r => r.id === id);
    if (index === -1) throw new Error('Request not found');

    // Create a copy to mutate to avoid "read only property" errors if Redux froze the previous reference
...
const changeStatus = async (id: string, status: string): Promise<ProductPriceReq> => {
    ...
    const req = { ...requests[index], status };
    requests[index] = req;
    return { ...req };
};
