...
export function IncshipmentEditScreen() {
    ...
    const onPressReceive = () => {
        const data_barang = selectedDetailIds.map(detailId => ({ id: detailId }));
        const errorMsg = validateReceive(data_barang);
        if (errorMsg) {
            setToast({
                visible: true,
                type: 'error',
    ...
    const handleConfirmModal = async () => {
        ...
        if (type === 'assign') {
            ...
            try {
                const data_barang = selectedDetailIds.map(detailId => ({ id: detailId }));
                await handleReceiveGoods(id, data_barang);
                await loadDetail(id); // Refresh detail data after receiving
                ...
                {isLoadingDetail || isRefreshing ? (
                    ...
                    <Animated.View key="content" entering={FadeInUp.duration(400)}>
                        ...
                        <View className="flex-row flex-wrap mb-4">
                            ...
                            {showPrintNavBtn && (
                                <TouchableOpacity
                                    onPress={() => navigation.navigate('IncshipmentPrintScreen', { id })}
                                    className="bg-indigo-500 px-4 py-2 rounded-lg mr-2 mb-2"
                                >
                                    <Text className="text-white font-bold text-sm">
                                        {showFirstPrintBtn ? 'Preview Barcode' : 'Print Barcode'}
                                    </Text>
                                </TouchableOpacity>
                            )}
                            {showReceiveBtn && (
                            ...
                            <View className="p-6 border-b border-gray-100">
                                <View className="flex-row justify-between mb-4 pb-4 border-b border-gray-100">
                                    ...
                                    <View className="flex-1 items-end">
                                        ...
                                        <View className={`px-2 py-1 rounded-md border ${isReceived ? 'bg-emerald-100 border-emerald-200' :
                                            (selectedItem.status_incoming === 'READY TO RECEIVE' && selectedItem.f_assign_barcode === 0) ? 'bg-red-50 border-red-200' :
                                                'bg-gray-100 border-gray-300'
                                            }`}>
                                            <Text className={`text-xs font-bold ${isReceived ? 'text-emerald-700' :
                                                (selectedItem.status_incoming === 'READY TO RECEIVE' && selectedItem.f_assign_barcode === 0) ? 'text-red-600' :
                                                    'text-gray-800'
