...
export type NotifModalType = 'terima' | 'batal';

interface NotifModalListProps {
    visible: boolean;
    type: NotifModalType;
    onDismiss: () => void;
...
export const NotifModalList = ({ visible, type, onDismiss, onConfirm }: NotifModalListProps) => {
    const [date, setDate] = useState(new Date());
    const [showDatePicker, setShowDatePicker] = useState(false);
    const [reason, setReason] = useState('');

    const isTerima = type === 'terima';
            ...
            >
                <View className="bg-white rounded-3xl w-10/12 p-8 items-center shadow-2xl mx-4 relative overflow-hidden" style={{ maxHeight: '90%' }}>
                    ...
                    <Text className="text-2xl font-bold text-slate-800 mb-2 text-center mt-2">
                        {isTerima ? 'Yakin menerima ?' : 'Yakin membatalkan ?'}
                    </Text>

                    {isTerima && (
                        <Text className="text-lg text-gray-500 mb-6 text-center">
                            Masukkan Tanggal Terima
                        </Text>
                    )}
                    ...
                    <View className="flex-row justify-center w-full mt-4">
                        <TouchableOpacity
                            onPress={() => onConfirm({ date, reason: isTerima ? undefined : reason })}
