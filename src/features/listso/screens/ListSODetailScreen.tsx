...
export function ListSODetailScreen() {
    ...
    const grandTotal = currentDetail?.items?.reduce((acc, item) => {
        const value = item.ntot_product_price_netto || item.subtotal || 0;
                            ...
                            <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mb-4">
                                <View>
                                    ...
                                    <View className="flex-row bg-gray-200 rounded-t-xl overflow-hidden border border-gray-200">
                                        ...
                                        <Text className="w-28 py-3 px-2 font-bold text-[11px] text-gray-700">Kode Barang</Text>
                                        <Text className="w-64 py-3 px-2 font-bold text-[11px] text-gray-700">Nama Barang</Text>
                                        ...
                                        <Text className="w-20 py-3 px-2 font-bold text-[11px] text-gray-700 text-center">Satuan</Text>
                                        <Text className="w-28 py-3 px-2 font-bold text-[11px] text-gray-700 text-center">Delivery Term</Text>
                                        <Text className="w-32 py-3 px-2 font-bold text-[11px] text-gray-700 text-right">Line Total</Text>
                                    </View>
                                    ...
                                    {currentDetail.items.map((item, index) => {
                                        ...
                                        return (
                                            <View key={index} className={`flex-row items-center border-b border-gray-200 border-x ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                                                ...
                                                <View className="w-28 py-2 px-2 justify-center">
                                                    <Text className="text-gray-900 text-[11px]">{item.code_product}</Text>
                                                </View>
                                                <View className="w-64 py-2 px-2 justify-center">
                                                    <Text className="text-gray-900 text-[11px]">{item.nm_product}</Text>
                                                </View>
                                                ...
                                                <Text className="w-20 py-3 px-2 text-[11px] text-gray-800 font-medium text-center">{item.nm_product_satuan}</Text>
                                                <Text className="w-28 py-3 px-2 text-[11px] text-gray-800 font-medium text-center">{item.delivery_term || '-'}</Text>
                                                <Text className="w-32 py-3 px-2 text-gray-800 text-[11px] text-right">{formatCurrency(item.ntot_product_price_netto || item.subtotal || 0)}</Text>
                                            </View>
                                        );
                                    })}
                                    {/* Grand Total Row */}
                                    <View className="flex-row bg-gray-50 border-b border-gray-200 border-x rounded-b-xl items-center">
                                        <View style={{ width: (10 + 28 + 64 + 36 + 32 + 16 + 20 + 28) * 4 }} className="py-3 px-2 justify-center">
                                            <Text className="font-bold text-[11px] text-gray-700">TOTAL</Text>
                                        </View>
                                        <View className="w-32 py-3 px-2 justify-center">
                                            <Text className="font-bold text-[11px] text-gray-700 text-right">{formatCurrency(grandTotal)}</Text>
                                        </View>
                                    </View>
                                </View>
                            </ScrollView>
