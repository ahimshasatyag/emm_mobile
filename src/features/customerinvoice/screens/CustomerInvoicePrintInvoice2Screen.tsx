...
export const CustomerInvoicePrintInvoice2Screen = () => {
    ...
    if (error && !detail) {
        return (
            <View style={{ flex: 1, backgroundColor: '#f9fafb' }}>
                <HeaderNavigator title="PRINT INVOICE V2" showBackButton onBackPress={() => navigation.goBack()} />
    ...
    const renderContent = () => {
        ...
        // DP payments (f_dp = 1, status CAIR/bukan cancel)
        const payments: any[] = detail.invoice_dtl || detail.payments || [];
        const dpPayments = payments.filter((p: any) => String(p.f_dp) === '1' && String(p.f_cancel) !== '1');
        const totalDP = dpPayments.reduce((sum: number, p: any) => sum + Number(p.v_amount || 0), 0);
        const pelunasan = total - totalDP;
        ...
        return (
            <ScrollView style={S.page} showsVerticalScrollIndicator={false}>
                ...
                <View style={S.tableContainer}>
                    ...
                    {/* Summary: SUBTOTAL always shown in V2 */}
                    ...
                    {/* DP rows */}
                    {dpPayments.map((p: any, idx: number) => (
                        <View key={p.id_invoice_dtl || idx} style={S.summaryRow}>
                            <Text style={S.summaryLabel}>DP TGL {fmtDate(p.date_cair)}</Text>
                            <View style={S.summaryValue}>
                                <View style={S.currencyAmount}>
                                    <Text style={{ fontSize: 11 }}>RP</Text>
                    ...
                    <View style={[S.summaryRow, { borderTopWidth: 1, borderColor: '#000' }]}>
                        <Text style={[S.summaryLabel, { fontWeight: '900' }]}>PELUNASAN</Text>
                        <View style={S.summaryValue}>
                            <View style={S.currencyAmount}>
                                <Text style={{ fontSize: 11, fontWeight: '700' }}>{currLabel}</Text>
                                <Text style={{ fontSize: 11, fontWeight: '700' }}>{fmtNum(pelunasan, isUSD)}</Text>
                            </View>
                        </View>
                    </View>
                </View>
                ...
                <View style={S.footerSection}>
                    <Text style={[S.footerText, { marginTop: 12 }]}>
                        ...
                        {': Rp. '}{pelunasan.toLocaleString('id-ID', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </Text>
                    <Text style={S.footerText}>
                        <Text style={S.footerBold}>TERBILANG</Text>
                        {': '}{formatTerbilang(pelunasan)}
                    </Text>
