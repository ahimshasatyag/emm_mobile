...
export const CustomerInvoiceInvLeasingDetailScreen = () => {
    ...
    if (error && !detail) {
        return (
            <View style={{ flex: 1, backgroundColor: '#f9fafb' }}>
                <HeaderNavigator title="PRINT INVOICE LEASING" showBackButton onBackPress={() => navigation.goBack()} />
        ...
        return (
            <ScrollView style={S.page} showsVerticalScrollIndicator={false}>
                ...
                <View style={S.tableContainer}>
                    ...
                    {/* Current Payment Ref with formatted date */}
                    {currentPayment && (
                        <View style={S.summaryRow}>
                            <Text style={[S.summaryLabel, { fontWeight: '900', textAlign: 'right' }]}>
                                {currentPayment.payment_ref} {currentPayment.date_cair ? `${String(currentPayment.nm_payment_method).toUpperCase()} TGL ${tglIndo(currentPayment.date_cair).toUpperCase()}` : ''}
                            </Text>
                            <View style={S.summaryValue}>
                                <View style={S.currencyAmount}>
