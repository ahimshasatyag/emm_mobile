...
const fmtDateSlash = (dateStr?: string | null) => {
    if (!dateStr) return '-';
    const d = new Date(dateStr);
    if (isNaN(d.getTime())) return '-';
    const day = String(d.getDate()).padStart(2, '0');
...
/**
 * CustomerInvoicePrintTTScreen
 * Mirrors vformpdf_td.php — Tanda Terima V1
 * Shows: invoice summary table + payment detail (with kurs line if USD) + rincian pembayaran list
 */
export const CustomerInvoicePrintTTScreen = () => {
    ...
    if (error && !detail) {
        return (
            <View style={{ flex: 1, backgroundColor: '#f9fafb' }}>
                <HeaderNavigator title="PRINT TANDA TERIMA 1" showBackButton onBackPress={() => navigation.goBack()} />
    ...
    const renderContent = () => {
        ...
        // Get first unpaid invoice detail (date_cair is null)
        const allPayments: any[] = detail.invoice_dtl || detail.payments || [];
        const invoiceTerakhir = allPayments.find((p: any) => !p.date_cair && String(p.f_cancel) !== '1');
        const kurs = invoiceTerakhir ? Number(invoiceTerakhir.nkurs || 0) : 0;
        const nominal = invoiceTerakhir ? Number(invoiceTerakhir.v_amount || 0) : 0;
        const paymentRef = invoiceTerakhir?.payment_ref || '';
        ...
        return (
            <ScrollView style={S.page} showsVerticalScrollIndicator={false}>
                ...
                <View style={S.tableContainer}>
                    ...
                    {/* Payment ref row (next unpaid payment) */}
                    {nominal > 0 && (
                        <>
                            <View style={S.summaryRow}>
                                <Text style={[S.summaryLabel, { fontWeight: '900' }]}>{paymentRef}</Text>
                                <View style={S.summaryValue}>
                                    <View style={S.currencyAmount}>
                                        <Text style={{ fontSize: 11, fontWeight: '700' }}>{currLabel}</Text>
                                        <Text style={{ fontSize: 11, fontWeight: '700' }}>{fmtNum(nominal, isUSD)}</Text>
                                    </View>
                                </View>
                            </View>

                            {/* Kurs conversion if USD */}
                            {kurs > 0 && (
                                <View style={S.summaryRow}>
                                    <Text style={[S.summaryLabel, { fontWeight: '700' }]}>
                                        USD {fmtNum(nominal, true)} * RP {fmtNum(kurs, false)}
                                    </Text>
                                    <View style={S.summaryValue}>
                                        <View style={S.currencyAmount}>
                                            <Text style={{ fontSize: 11, fontWeight: '700' }}>RP</Text>
                                            <Text style={{ fontSize: 11, fontWeight: '700' }}>
                                                {(nominal * kurs).toLocaleString('id-ID', { minimumFractionDigits: 2 })}
                                            </Text>
                                        </View>
                                    </View>
                                </View>
                ...
                <View style={[S.footerSection, { marginTop: 12 }]}>
                    ...
                    {rincianPayments.map((p: any, idx: number) => {
                        ...
                        if (p.payment_ref && String(p.payment_ref).trim().length > 0) {
                            textRincian += ` (${p.payment_ref} ${fmtDateSlash(p.date_draft)}`;
                            if (p.code_giro) textRincian += ` BG ${p.code_giro}`;
                            textRincian += `)`;
                        }
